  ...
  return (
    <button
      ...
      className={clsx(
        ...
        copied
          ? "bg-purple-400/10 ring-1 ring-inset ring-purple-400/20"
    ...
    >
      ...
      <span
        aria-hidden={!copied}
        className={clsx(
          "pointer-events-none absolute inset-0 flex items-center justify-center text-purple-400 transition duration-300",
          !copied && "translate-y-1.5 opacity-0"
        )}
      >
        Copied!
...
function CodePanelHeader({ tag, label }: { tag?: string; label?: string }) {
  ...
  return (
    <div className="border-b-white/7.5 dark:bg-white/1 flex h-9 items-center gap-2 border-y border-t-transparent bg-zinc-300 px-4 dark:border-b-white/5 dark:bg-zinc-900">
      ...
      {label && <span className="font-mono text-xs text-zinc-600 dark:text-zinc-400">{label}</span>}
    </div>
  );
}
            ...
            <Tab
              className={clsx(
                ...
                childIndex === selectedIndex
                  ? "border-purple-700 text-purple-600 dark:border-purple-500 dark:text-purple-400"
