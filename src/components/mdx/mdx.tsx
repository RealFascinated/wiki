...
import { TabGroup, TabList, Tab as TabListItem, TabPanel, TabPanels } from "@headlessui/react";
...
export { Code as code, CodeGroup, Pre as pre } from "@/components/mdx/code";
...
export function Warning({ children }: { children: React.ReactNode }) {
  return (
    <div className="my-6 flex gap-2.5 rounded-2xl border border-purple-500/20 bg-purple-50/50 p-4 leading-6 text-purple-900 dark:border-purple-500/30 dark:bg-purple-500/5 dark:text-purple-200 dark:[--tw-prose-links-hover:theme(colors.red.300)] dark:[--tw-prose-links:theme(colors.white)]">
      <ExclamationCircleIcon className="mt-1 h-4 w-4 flex-none fill-purple-500" />
      <div className="[&>:first-child]:mt-0 [&>:last-child]:mb-0">{children}</div>
    </div>
  );
}

export function Tabs({ children, tabs }: { children: React.ReactNode; tabs: string[] }) {
  return (
    <TabGroup>
...
export function Properties({ children }: { children: React.ReactNode }) {
  return (
    <div className="my-6">
      <ul role="list" className="m-0 list-none divide-y divide-zinc-900/5 p-0 dark:divide-white/5">
        {children}
      </ul>
    </div>
  );
        ...
        {type && (
          <>
            ...
            <dd className="font-mono text-xs text-zinc-400 dark:text-zinc-500">
              {required && <Tag className="mr-3 py-1" color="red">{`Required`}</Tag>}
              {optional && <Tag className="mr-3 py-1" color="sky">{`Optional`}</Tag>}
              {defaultValue && (
                <Tag className="mr-3 py-1" color="zinc">{`Default: ${defaultValue}`}</Tag>
              )}
              {since && <Tag className="py-1" color="amber">{`Since ${since}`}</Tag>}
              {deprecated && <Tag className="py-1" color="red">{`DEPRECATED`}</Tag>}
            </dd>
          </>
        )}
